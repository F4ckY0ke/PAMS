...
try:
   ...
   APP_ID = results[0][2]
   API_KEY = results[0][3]
   SECRET_KEY = results[0][4]
   face_score = results[0][9]
   QRAPI_KEY = results[0][5]
   QRSECRET_KEY = results[0][6]
   Access_token = results[0][7]
   # 从数据库取，失效的话在下面更新到数据库
except:
   print("Error: 无法生成数据")
        ...
        if 'error_code' in list1:
            if list1['error_code'] == 0:
                ...
                if maxscore >= face_score:
                    ...
                    db = MySQLdb.connect(DBIP, DBID, DBPWD, DBNAME, charset='utf8')
                    cursor = db.cursor()
                    sql = "SELECT USERFLAG FROM U_LOCAL WHERE FACENAME = '" + checkFACENAME + "';"
                    cursor.execute(sql)
                    USERFLAG = cursor.fetchone()
                    db.close()
                    if USERFLAG[0] == 1:
                        ...
                        self.FACEIMG.setText("人脸认证失败")  # 发送开门指令
                        self.FACETIP.setText("")
...
def QRCoder(Access_token):
    ...
    if response:
        ...
        if 'error_code' in jsonCode:  # 判断错误接口返回是否有错误代码
            ...
            cursor = db.cursor()
            # SQL 查询语句
            sql = "UPDATE A_XTCANSHU SET ACCESS_TOKEN = '" + rsAcc + "';"
            try:
                # 执行SQL语句
                cursor.execute(sql)
                # 提交到数据库执行
            ...
            db = MySQLdb.connect(DBIP, DBID, DBPWD, DBNAME, charset='utf8')
            cursor = db.cursor()
            sql = "SELECT CODEFLAG FROM U_VISITOR WHERE RANDOM = '" + checkRANDOM + "';"
            cursor.execute(sql)
            CODEFLAG = cursor.fetchone()
            db.close()
            global QRRS
            QRRS = CODEFLAG[0]  # 查询数据库验证用户合法性
    else:
        restart_program()
