	...
	$('#babyTable').DataTable( {//发送请求把返回的json数据输出到表格babyTable
		...
		"columns": [//指定列数据
		  ...
		  "createdRow": function ( row, data, index) {
			...
			var $btn = $('<div class="btn-group text-cen">'+//获取当前行第3列内数据做拼接后生成按钮
			    '<input type="button" class="btn btn-danger" onclick="sendDel(\''+row.cells[0].innerHTML+'\',\''+row.cells[1].innerHTML+'\')" value="删除">'+
			    '</div>'+
			    '</div>');
...
<div class="panel panel-default">
    ...
    <div class="panel-body">
      ...
      <tr>
        ...
        <th align="center" valign="middle" class="borderright">人脸唯一标识</th>
...
function sendDel(facename, token){
	var con;
	con = confirm("确认删除识别码为"+token+"的人脸数据么？");//弹出确认对话框
	if(con==true){//单击确认按钮布尔变量为true
		window.location.href = "delface.php?facename="+facename+"&token="+token+"";
	}
	else window.location.href = "facetable.php";//单击取消按钮刷新页面
